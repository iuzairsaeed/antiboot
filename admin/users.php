...
<head>
	...
	<title><?php echo $web_title;?>Manage Users</title>
			...
			<div class="row">
				...
				<div class="col-sm-12">
					<section class="panel" style = "box-shadow:4px 5px 4px rgba(0, 0, 0, .5);">
						...
						<tbody>
							<?php 
								$SQLGetUsers = $odb -> prepare("SELECT * FROM `users`");
								$SQLGetUsers -> execute();
								while ($getInfo = $SQLGetUsers -> fetch(PDO::FETCH_ASSOC))
								{
									...
									if ($status == 0)
									{
										...
										$status = '<span class="label label-success">Banned</span>'; 
									}
									echo '
									<tr class="gradeX">
										<td>('.$id.') '.$username.'</td>
										<td>'.$email.'</td>
										<td>'.$status.'</td>
										<td class="center"><a href="edituser.php?id='.$id.'"><button class="btn btn-primary" type="button">Edit User</button></a></td>
									</tr>';
								}
							?>
						</tbody>
